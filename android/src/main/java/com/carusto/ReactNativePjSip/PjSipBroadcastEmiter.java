...
public class PjSipBroadcastEmiter {
    ...
    public void fireCallReceivedEvent(PjSipCall call) {
        ...
        try {
            String[] nameAndNumber = PjSipUtils.getNameAndNumberFromUri(data.getString("remoteUri"));
            String callerName = nameAndNumber[0];
            String callerNumber = nameAndNumber[1];
            if(callerName == null || callerName.isEmpty()) {
                callerName = callerNumber;
            }

            String uuid = data.getString("callId");

            if(RNCallKeepModule.instance != null && PjSipModule.instance != null) {
                RNCallKeepModule.instance.displayIncomingCall(uuid, callerName, callerNumber, false);
            }
        } catch (Exception e) {
            Log.e(TAG, "Failed to display incoming call event", e);
        }
